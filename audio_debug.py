...
while True:
    ...
    # Normalize pixel values if using a floating mode
    ...
    print("Boxes:", output_details[boxes_idx]['name'])                                              
    classes = interpreter.get_tensor(output_details[classes_idx]['index'])[0] # Class index of detected objects
    scores = interpreter.get_tensor(output_details[scores_idx]['index'])[0] # Confidence of detected objects

    # Loop over all detections and draw detection box if confidence is above minimum threshold
    for i in range(min(len(scores), len(boxes), len(classes))):
        if any((scores[i] > min_conf_threshold) and (scores[i] <= 1.0) for score in scores):
